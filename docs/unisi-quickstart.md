...
5. Custom web client: set `web_client = 'path/to/your/client'` in `config.py` to serve your own UNISI-protocol front end at `/` instead of the bundled one (which then stays available at `/default`). See [§18 of the Programming Spec](./unisi-programming-spec.md#18-custom-web-client-configweb_client).
